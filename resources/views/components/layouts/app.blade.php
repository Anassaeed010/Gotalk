<x-layouts.default>
    ...
    <aside id="sidebarMenu"
        ...
        <ul class="flex flex-col gap-3">
            <li><a href="{{ route('home') }}" class="flex items-center gap-2 font-semibold text-white px-4 py-3 rounded-lg hover:bg-purple-700 transition">🏠 الرئيسية</a></li>
            <li><a href="{{ route('explore') }}" class="flex items-center gap-2 font-semibold text-white px-4 py-3 rounded-lg hover:bg-purple-700 transition">🔍 استكشاف</a></li>
            @guest
                <li><a href="{{ route('login') }}" class="flex items-center gap-2 font-semibold text-white px-4 py-3 rounded-lg hover:bg-purple-700 transition">👤 تسجيل دخول</a></li>
            @endguest
            @auth
                <li>
                    <form method="post" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="flex items-center gap-2 font-semibold text-white px-4 py-3 w-full text-left rounded-lg hover:bg-purple-700 transition">🚪 تسجيل خروج</button>
                    </form>
                </li>
            @endauth
        </ul>
    ...
    @auth
        <form method="post" action="{{ route('tweet.create') }}"
            ...
            <button type="submit" class="bg-purple-500 hover:bg-purple-600 p-2 rounded-lg text-white font-bold">🚀</button>
        </form>
    @endauth
