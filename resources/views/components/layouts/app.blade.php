<x-layouts.default>
  ...
  <nav class="navbar bg-black text-white rounded-box justify-between gap-4 shadow-base-300/20 shadow-sm">
    ...
    <!-- Navbar Center -->
    <!-- Navbar Center مع خط عربي عصري -->
<div class="navbar-center flex items-center justify-center">
  <a href="{{ route('home') }}"
     class="text-4xl md:text-6xl font-extrabold no-underline
  ...
  <aside id="sidebarMenu"
    ...
    <ul class="flex flex-col gap-3">
      ...
      <li>
        <a href="/explore" class="flex items-center gap-2 font-semibold text-white px-4 py-3 rounded-lg hover:bg-purple-700 transition">
          🔍 استكشاف
        </a>
      </li>
      @guest
      ...
      <li>
        <form method="post" action="{{ route('logout') }}">
          @csrf
          <button type="submit" class="flex items-center gap-2 font-semibold text-white px-4 py-3 w-full text-left rounded-lg hover:bg-purple-700 transition">
            🚪 تسجيل خروج
          </button>
        </form>
      </li>
  ...
  <div class="space-y-4">
    ...
    <div class="bg-gray-900 p-4 rounded-lg hover:bg-gray-800 transition shadow-lg">
      <p>{{ $tweet->content }}</p>
